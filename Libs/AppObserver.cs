...
namespace DouSlackingMonitor.Libs
{
    ...
    {
        private IntPtr hook;
        readonly Win32API.WinEventDelegate winEventDelegate;

        public event AppObserverEventHandler OnAppActive;
        ...
        private async void Handle(IntPtr hwnd)
        {
            ...
            var processResult = await Task.Run(() =>
            {
                ...
                {
                    var str = e.Message + " | Process Name:" + processName + " | Process File:" + processFileName + " | Process Description:" + processDescription;
                    Console.WriteLine(str.Pastel(Color.IndianRed));
                }
                return processName;
            });
        ...
        private string[] GetProcessInfoByWindowHandle(IntPtr hwnd)
        {
            ...
            {
                Console.WriteLine($"无法获取进程：{processName}的句柄，请尝试以管理员身份运行Dou Slacking Monitor。".Pastel(Color.IndianRed));
            }
